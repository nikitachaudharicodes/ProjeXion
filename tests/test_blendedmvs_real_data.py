...
def lowres_dir(tmp_path):
    ...
    into a temp dir so tests do not clobber your real data.
    """
...
def test_length_and_item_shapes(lowres_dir):
    ...
    assert len(ds) == 2  # your sample has exactly 2 “train” objects
    item = ds[0]
    # dataset must return exactly 6 things:
    assert isinstance(item, tuple) and len(item) == 6
    imgs, depth, K_ref, K_src, Rt_ref, Rt_src = item
